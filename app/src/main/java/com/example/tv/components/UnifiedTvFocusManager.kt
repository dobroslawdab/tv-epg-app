        ...
        when (event.key) {
            Key.DirectionLeft -> {
                if (_channelContentState.focusedColIndex > 0) {
                    ...
                    channelFocusRequesters[Pair(_channelContentState.focusedRowIndex, newColIndex)]?.requestFocus()
                } else {
                    // Go back to CategoryIcon
                    _channelContentState = _channelContentState.copy(focusedColIndex = -1)
                    channelFocusRequesters[Pair(_channelContentState.focusedRowIndex, -1)]?.requestFocus()
                    switchLevel(FocusLevel.CHANNEL_MENU)
                }
                return true
            }
