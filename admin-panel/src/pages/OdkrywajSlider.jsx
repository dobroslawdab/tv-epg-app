...
function OdkrywajSlider() {
  ...
  const KINO_PLAY_LOGO = 'https://kexrkaqxoadxugnnbnjh.supabase.co/storage/v1/object/public/odkrywaj-slider/kino_play_logo.png';
    ...
    try {
      ...
      if (uploadError) {
        ...
        if (uploadError.message.includes('bucket') || uploadError.statusCode === '404') {
          throw new Error('Bucket "odkrywaj-slider" nie istnieje. Utwórz go w Supabase Storage.');
        }
  ...
  const saveSlide = async () => {
    ...
    try {
      ...
      // (movies table might use numeric IDs, but odkrywaj_slider expects UUID)
  ...
  const openEditModal = (item) => {
    setFormData({
      ...
      button_type: item.button_type || 'rent',
  ...
  const getButtonLabel = (type, customText) => {
    switch (type) {
      case 'rent': return 'Wypożycz';
      case 'watch': return 'Oglądaj';
  ...
  return (
    <div className="odkrywaj-slider-page">
      <h1>ODKRYWAJ Slider - Zarządzanie</h1>
      <p className="page-description">
        Zarządzaj slajdami wyświetlanymi na stronie głównej ODKRYWAJ. Możesz dodać filmy z bazy, materiały wideo lub ręcznie utworzone slajdy.
      </p>
                ...
                <div className="button-options">
                  <label>
                    <input
                      type="radio"
                      name="button_type"
                      checked={formData.button_type === 'rent'}
                      onChange={() => setFormData({ ...formData, button_type: 'rent' })}
                    />
                    🛒 Wypożycz
                  </label>
                  <label>
                    <input
                      type="radio"
                      name="button_type"
                      checked={formData.button_type === 'watch'}
                      onChange={() => setFormData({ ...formData, button_type: 'watch' })}
                    />
                    ▶️ Oglądaj
                  </label>
                  <label>
